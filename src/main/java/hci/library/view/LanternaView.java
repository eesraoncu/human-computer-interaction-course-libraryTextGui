...
public class LanternaView implements ConsoleView {
    ...
    public void initMainScreen(Runnable onViewBooks, Runnable onSearchBook, Runnable onBorrowBook, Runnable onReturnBook, Runnable onAddBook,
        ...
        leftPanel.addComponent(new Button("Search Book", onSearchBook));
        leftPanel.addComponent(new Button("Add Book", onAddBook));
        leftPanel.addComponent(new Button("Delete Book", onDeleteBook));
        leftPanel.addComponent(new Button("Borrow Book", onBorrowBook));
        leftPanel.addComponent(new Button("Return Book", onReturnBook));
        leftPanel.addComponent(new Button("Switch Mode", () -> {
            onSwitchView.run();
            if (mainWindow != null)
                mainWindow.close();
        }));
